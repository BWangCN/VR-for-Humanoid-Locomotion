# batch_glb_to_usd_and_bbox.py  (living room)
# Run:
#   blender -b -P livingroom/batch_glb_to_usd_and_bbox.py --% -- --input ".\livingroom\livingroom_assets_raw" --output ".\livingroom\livingroom_assets_usd"
#
...
# Realistic size ranges (meters) for each living room class
CLASS_RANGES = {
    "sofa":         {"x": (1.8, 2.8), "y": (0.7, 1.1), "z": (0.7, 1.0)},
    "tv_stand":     {"x": (1.0, 2.0), "y": (0.35, 0.55), "z": (0.4, 0.7)},
    "bookshelf":    {"x": (0.6, 1.5), "y": (0.25, 0.45), "z": (1.2, 2.2)},
    "cabinet":      {"x": (0.8, 1.8), "y": (0.35, 0.55), "z": (0.7, 1.2)},
    "desk":         {"x": (0.8, 1.6), "y": (0.4, 0.9),  "z": (0.65, 0.9)},
    "piano":        {"x": (1.3, 1.6), "y": (0.55, 0.70), "z": (1.1, 1.3)},
    "armchair":     {"x": (0.7, 1.0), "y": (0.7, 1.0),  "z": (0.7, 1.1)},
    "coffee_table": {"x": (0.8, 1.4), "y": (0.5, 0.9),  "z": (0.3, 0.55)},
    "floor_lamp":   {"x": (0.2, 0.5), "y": (0.2, 0.5),  "z": (1.2, 1.8)},
    "side_table":   {"x": (0.3, 0.6), "y": (0.3, 0.6),  "z": (0.4, 0.7)},
    "plant_pot":    {"x": (0.15, 0.50), "y": (0.15, 0.50), "z": (0.20, 0.80)},
    "cushion":      {"x": (0.35, 0.60), "y": (0.35, 0.60), "z": (0.08, 0.20)},
    "blanket":      {"x": (1.2, 2.0),   "y": (1.0, 1.8),   "z": (0.01, 0.06)},
    "book":         {"x": (0.15, 0.30), "y": (0.20, 0.35), "z": (0.02, 0.08)},
    "magazine":     {"x": (0.20, 0.28), "y": (0.26, 0.35), "z": (0.003, 0.015)},
    "remote":       {"x": (0.04, 0.06), "y": (0.15, 0.25), "z": (0.02, 0.04)},
    ...
    "toy":          {"x": (0.10, 0.40), "y": (0.10, 0.40), "z": (0.10, 0.40)},
}
...
CLASS_HEIGHT_TIERS = {
    "blanket": [
        (0.50, (0.01, 0.03)),   # very flat  — diff 1 suitable
        (0.30, (0.02, 0.05)),   # thin       — diff 1 suitable
        (0.20, (0.04, 0.06)),   # thicker    — diff 3 suitable
    ],
    "shoes": [
    ...
    "cushion": [
        (0.40, (0.06, 0.10)),   # flat cushion  — diff 1 suitable
        (0.35, (0.08, 0.15)),   # medium        — diff 3 suitable
        (0.25, (0.12, 0.20)),   # thick         — diff 3 suitable
    ],
    "book": [
    ...
    "magazine": [
        (0.60, (0.003, 0.008)),  # single issue  — very thin
        (0.40, (0.008, 0.015)),  # small stack   — still thin
    ],
    "remote": [
        (0.55, (0.02, 0.03)),   # slim remote
        (0.45, (0.025, 0.04)),  # chunky remote
    ],
    "toy": [
        (0.40, (0.08, 0.12)),   # small toy — diff 1 suitable
        (0.35, (0.10, 0.18)),   # medium toy — diff 3 suitable
        (0.25, (0.15, 0.25)),   # large toy — non-step-over obstacle
    ],
}
...
def parse_args():
    ...
    p.add_argument("--input", required=True, help="Root folder like livingroom_assets_raw/")
    p.add_argument("--output", required=True, help="Root folder like livingroom_assets_usd/")
